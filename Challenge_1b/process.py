...
def process_collection(collection_path, model):
    ...
    # Step 4: Score Relevance Using Embeddings
    ...
    # Step 5: Select Top Sections
    # We filter out sections below the similarity threshold before finding the top N
    scores_above_threshold = cosine_scores > SIMILARITY_THRESHOLD
    
    # Get the indices and scores of sections that are above the threshold
    indices_above_threshold = torch.where(scores_above_threshold)[0]
    scores_filtered = cosine_scores[scores_above_threshold]

    # Determine the number of sections to retrieve (up to the limit)
    ...
    # Collect the top sections' metadata
    extracted_sections = []
